...
if ($access_token) {
    try {
        ...
        if ($decoded) {
            $sql = "SELECT p.id, p.name as nameProduct, p.price, p.images, f.is_like
            FROM favorites f JOIN products p ON f.id_product = p.id
            WHERE f.id_user = (SELECT id from users WHERE username = '$username')
            ORDER BY f.id_product";

            $favoriteList = executeResult($sql);
